...
2. `AddIn.GetCustomUI` reads `ribbon.xml`. The canonical copy is **embedded** in `NoteHighlightAddin.dll` via `Properties\Resources.resx` (exposed as `Resources.ribbon`); a sibling `ribbon.override.xml` file next to the DLL, if present, is preferred and used instead as a dev / diagnostic hot-edit convenience. End-user MSIs do not ship `ribbon.override.xml` - the override path is dev-only by construction. A legacy `ribbon.xml` next to the DLL (left behind by pre-3.9 installs whose MSI marked the file `Permanent=TRUE`) is deliberately IGNORED with a `Trace.TraceWarning` naming the resolved path; users may delete it manually but the add-in no longer reads it. The embedded copy is the load-bearing one because the DLL is versioned and always replaces on MSI upgrade, unlike the unversioned on-disk file (bug 13.1, fixed 2026-05-13 - see `.local/docs/bugs/msi-ribbon-stale-on-upgrade.md`). Each language button has a `tag` attribute that maps to a `highlight` syntax key (e.g. `cs`, `py`, `ps1`).
...
Add a `<button>` in `NoteHighlightAddin/ribbon.xml` with `onAction="AddInButtonClicked"`, `tag="<highlight syntax key>"`, `image="<png in Resources\>"`, and `getVisible="GetLanguageButtonVisible"` (the picker controls visibility per-user; do not set `visible="..."` on language buttons). The image must be a short lowercase PNG named after the highlighter syntax key's idiomatic short alias (e.g. `cs.png` for C#, `kt.png` for Kotlin, `rs.png` for Rust, `ps1.png` for PowerShell) - this is the post-4.0 convention; the canonical set already in `NoteHighlightAddin/Resources/` is the template for any new addition. Each new language button must have its own distinct icon: `Other.png` is NOT a fallback for new language buttons, it is reserved for the `menuMoreLanguages` dynamic menu, the `buttonSettings` button and the `buttonLanguages` button only. Sharing `Other.png` across language buttons was the long-standing usability gap the 4.0 icon refresh closed (15 buttons all looked identical on the ribbon), so do not reintroduce it. Also add a matching `<Content Include="Resources\<name>.png">` row to `NoteHighlightAddin.csproj` so the MSI deploys the file. The ribbon file ships as an embedded resource only (canonical, picked up automatically because `Resources.resx` references `..\ribbon.xml` via `ResXFileRef`); a rebuild of `NoteHighlightAddin.dll` is enough - no separate deployment step. The MSI does not deploy `ribbon.xml` to disk on end-user installs (this changed in 3.9 to close the unversioned-file bug 13.1; see `.local/docs/bugs/msi-ribbon-stale-on-upgrade.md`). Dev hot-edits use a different filename - drop a `ribbon.override.xml` next to the DLL while iterating, then rebuild for the embedded copy to catch up. End users opt buttons in or out via the Languages... dialog (Phase 3) rather than by hand-editing `ribbon.xml`.

### `ribbon.xml` is the canonical language list

`NoteHighlightAddin/ribbon.xml` is the single source of truth for which languages the add-in offers. `LanguageRegistry` parses it at startup. Source order (post bug 13.1 followup, 2026-05-13 evening): an on-disk `ribbon.override.xml` next to the DLL is preferred if present - via `AddIn.GetAddinDirectory()` + `File.Exists` validation, never `Assembly.Location` directly; see `feedback_com_addin_path_traps.md` - and otherwise falls back to the embedded `Properties.Resources.ribbon` copy. A legacy `ribbon.xml` next to the DLL (from a pre-3.9 install whose MSI marked the file `Permanent=TRUE`) is deliberately IGNORED with a `Trace.TraceWarning` logging the resolved path. End-user installs never have `ribbon.override.xml` - the MSI does not ship it - so the override path is dev-only by construction. The parsed `<button>` set is exposed as `LanguageRegistry.All`. The per-user picker state (`%APPDATA%\NoteHighlight2016\languages.json`, managed by `LanguageSettings`) only ever **selects from** that set - it cannot introduce a language that ribbon.xml does not declare, and unknown tags persisted in `languages.json` are dropped silently with a `Trace.TraceWarning` on load. Build-time sanity checking against `highlight\langDefs\*.lang` is a separate concern (test-only); a tag with no matching `.lang` file or alias is logged once and not shown to the user. To add a new language, add it to `ribbon.xml` (above) - everything else flows from that.
